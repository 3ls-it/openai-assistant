...
2. Configure & Run  
   In the release directory, simply invoke the program; on first run it will auto-generate `~/.openai-assistant/settings.py` and prompt for required values. Optionaly (and recommended) copy `openai-assistant` to a location in your `PATH`, e.g., `~/bin/`, `~/.local/bin/`, etc.  
   ```bash
